...
\begin{frame}{Introdução}{Análise do \alert{Rastro de Proveniência} em \alert{Simulações Computacionais} em Larga Escala}
    ...
    \centerline{\textsc{Análise do \alert{Rastro de Proveniência} em \alert{Simulações Computacionais} em Larga Escala}}
...
\begin{frame}{O problema}
    \todox{3 tipos de consulta}
    \todox{Descrever o problema a ser resolvido}
    \vfill
    \setbeamercovered{invisible}
    \pause
    \centerline{\textit{\large ``Como submeter os três tipos de consulta anteriores?''}}
\end{frame}
...
\begin{enumerate}
\item Gerência de conteúdo científico $\leftarrow$ arquivos científicos
\item Indexação dos dados científicos
\item Correlação em um SGBD relacional
\begin{itemize}
\item Dados de domínio $\searrow$
\item Dados de proveniência do fluxo de dados
\end{itemize}
\end{enumerate}
\end{block}
...
\item Suporta os 3 tipos de consulta mencionados
\item \textit{On-line} e \textit{off-line}
\end{itemize}

\perrotta{Mais alguma sugestão?}
...
% \caption{
% \\ Cor branca: captura de dados de proveniência
% \\ Cor cinza: etapas de processamento
% }
\end{figure}

\begin{description}[Cor branca]
\item[Cor branca] a captura de dados de proveniência
\item[Cor cinza] as etapas de processamento
\end{description}
}
...
% \todox{QPP, Citar o objetivo do trabalho, descrever eventuais vantagens que serão buscadas}
...
\item[físico]  atributos relacionados à mesma instância de execução (\textit{taskid}) % / tarefa
\item[lógico]  atributos de domínio
\item[híbrido] \emph{ambos} os anteriores
\end{description}
\end{block}

\perrotta{Destacar o híbrido? Nossa proposta?}
}
...
\begin{itemize}
\item \alert{\textsc{[OK]}} Resultados esperados foram obtidos
\begin{itemize}
\item sintaticamente válidos
\end{itemize}
\item \alert{\textsc{[OK]}} Baixo \textit{overhead} de tempo para a geração das consultas $\rightarrow$ ordem de \texttt{ms}
\begin{itemize}
\item pré-processamento com as estruturas de dados % os \textit{hash multimaps}
\item idempotente
\end{itemize}
\item \alert{\textsc{[OK]}} intuitiva de usar
\end{itemize}
\end{block}
...
\item Baixo \textit{overhead} de execução
\end{itemize}
\end{alertblock}
}
